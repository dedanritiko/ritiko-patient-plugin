"""
Setup file for Ritiko Patient Plugin
"""
import json
import os
import sys
        ...
        if is_paid:
            ...
            if not license_key:
                ...
                self.announce(
                    ...
                    f"  pip install {PLUGIN_METADATA.get('id', 'ritiko-patient-plugin')}\n",
                    level=1,
                )
                self.announce("=" * 70 + "\n", level=1)
                sys.exit(1)
...
setup(
    name=PLUGIN_METADATA.get("id", "ritiko-patient-plugin"),
    version=PLUGIN_METADATA.get("version", "1.0.0"),
    description=PLUGIN_METADATA.get("description", "Patient Plugin"),
    ...
    url=PLUGIN_METADATA.get("url", "https://github.com/dedanritiko/ritiko-patient-plugin"),
    license="AGPL-3.0",
    ...
    keywords="ritiko plugin patient django",
)
